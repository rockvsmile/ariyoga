...
3. **Giao đúng agent** (xem bảng trong CLAUDE.md), truyền: id, khung, lời người dùng vừa nói. Khung 1: `dao-dien` rồi `bien-kich`. Khung 5: `bien-kich` rồi `dao-dien` duyệt nội bộ. Khung 6: `quay-phim`, rồi hỏi người dùng có muốn `hoa-si-storyboard` vẽ ảnh không (tốn credit) hay chỉ lấy prompt.
4. Sau khi agent xong: chạy lại `kiem_tra_du_an.py`. Tóm tắt cho người dùng 3–6 dòng: đã làm gì, lựa chọn chính, cần họ quyết định gì. Nhắc xem và duyệt trên bảng điều khiển.
5. Chế độ `tu_dong`: lặp lại bước 2–4 cho các khung chưa khoá, dừng trước khung 7.
