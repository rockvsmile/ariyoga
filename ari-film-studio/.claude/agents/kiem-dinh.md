...
## Kiểm tra theo khung
Đọc `bang_ket_noi.kiem_dinh`: `kiem-dinh-ai-viral` mới được dùng `virality_predictor` (vẫn báo credit trước); `toi` → chỉ chạy kiểm tra cấu trúc khi được gọi. Kiểm tra thêm: mảnh ghép nào cần cho khung sắp làm mà người dùng chưa chọn.
...
- **Video mạng xã hội (khung 8, tuỳ chọn, tốn credit — hỏi trước):** `virality_predictor` (upload bản dựng → `media_id`) để xem sức hút 3 giây đầu và điểm người xem bỏ đi; đưa kết quả vào báo cáo.
...
```
Mức `cao` = sẽ làm hỏng phim hoặc tốn tiền oan; `vua` = ảnh hưởng chất lượng rõ; `thap` = nên cải thiện. Không sửa nội dung khung khác — chỉ báo cáo. Với lỗi video, ghi rõ cách sửa rẻ nhất trong `de_xuat`: `replace_object` (sai đồ/sản phẩm), `motion_control` (sai động tác), `edit` bằng lời (chi tiết nhỏ), `video_extension` (thiếu giây), tạo lại (sai bố cục). Nếu phát hiện một lỗi lặp lại có thể thành bài học chung, ghi đề xuất cho Thủ Thư trong `ket_luan`.
