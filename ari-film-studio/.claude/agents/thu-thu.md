...
## Việc bạn làm
1. **Thêm phong cách từ phim mẫu:** nếu có Higgsfield, upload video (hoặc dùng link YouTube) → `video_analysis_create` → `video_analysis_status` để có phân tích từng cảnh (video ngắn cho kết quả chính xác nhất; báo người dùng chờ 3–5 phút). Kết hợp với tự trích khung hình (`python tools/ghep_phim.py --khung-video <video> <thu-muc-ra>` lấy 2 khung/giây), nghe âm thanh nếu cần, phân tích theo đúng khung 9 yếu tố + Shot list gốc + Công thức cấu trúc + Prompt template + Lỗi thường gặp — theo mẫu `thu-vien/phong-cach/01-quiet-luxury-kien-truc.md`. Lưu thành `thu-vien/phong-cach/NN-ten.md` (NN tăng dần). Học ngôn ngữ hình ảnh, không chép logo/tên thương hiệu.
2. **Thêm/cập nhật công cụ:** dùng `thu-vien/cong-cu/_mau.md`; tra cứu thông số trên mạng, ghi nguồn và tháng tra cứu. **Cập nhật danh mục Higgsfield:** gọi `models_explore` (`list`, type video/image/audio) và `get_workflow_instructions` (không tham số) → so với `thu-vien/cong-cu/higgsfield.md` và danh sách `model_video`/`model_anh` trong `tuy-chon.json`; báo model mới/bị bỏ và đề xuất cập nhật. Không đoán thông số — không tìm thấy thì ghi "chưa kiểm chứng".
